...
// Chơi MỘT truyện tiếng Anh trên bản đồ "Story in Order": Google TTS đọc lời kể bằng giọng
// Anh, khay tranh (đã xáo) ở dưới + các ô trống đánh số ở trên. Bé kéo/chạm để xếp tranh
// theo đúng thứ tự trước–sau. Đặt đúng → khen + pháo giấy + đọc câu tiếng Anh của khung ấy;
// đặt sai → rung nhẹ, tranh về khay. Xếp đúng HẾT = 1 ngôi sao + đánh dấu truyện đã xong
// (mở khoá truyện kế) rồi hiện lối sang truyện tiếp.
    ...
    return (
      <main className="wrap">
        <div className="panel" style={{ textAlign: "center", marginTop: 20 }}>
          ...
          <Link href="/english/story" className="btn" style={{ marginTop: 12 }}>
            ← Về bản đồ
          </Link>
        </div>
      </main>
...
function StoryPlay({ story }: { story: Story }) {
    ...
    (frame: StoryFrame, slot: number, x: number, y: number) => {
      ...
      if (r.order[slot] === fid) {
        ...
        if (next.every((v) => v != null)) {
          ...
          void speak(frame.line, "en"); // đọc câu tiếng Anh vừa đặt đúng — củng cố "nghe hiểu"
        }
      } else {
        playWrong();
        setWrongSlot(slot);
  ...
  const replay = () => void speak(story.narration, "en");
  ...
  return (
    <main className="wrap lt-wrap story-wrap">
      <div className="lt-top">
        <Link href="/english/story" className="pill">
          ← Bản đồ
        </Link>
      ...
      {chapter && (
        <p className="story-crumb">
          {chapter.title} · Truyện {ordinal}/{STORY_ORDER.length} · {story.titleVi}
        </p>
      )}
        ...
        <div className="modal-back" role="dialog" aria-modal="true">
          <div className="modal result-modal lt-result">
            ...
            <div className="lt-result-actions">
              {nextStory ? (
                <Link href={`/english/story/${nextStory.id}`} className="btn">
              ...
              <Link href="/english/story" className="btn-ghost">
                Về bản đồ
              </Link>
            </div>
          </div>
